// src/components/public/SalesListing.tsx
import { useState, useRef, useEffect } from "react";
import { Heart, Share2, MapPin, Bed, Bath, Maximize2, Car, ChevronLeft, ChevronRight, Home, Check, Image as ImageIcon, DollarSign, TrendingUp, Shield, Target, Percent } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { usePublicSalesProperty, usePublicSimilarSalesProperties } from "../../store/query/property.queries";
...
// Helper to generate badge gradient
const generateBadgeGradient = (color: string) => {
...
export function SalesListing() {
  ...
  const id = salesId ? parseInt(salesId) : null;
  ...
  const pricePerSqmRef = useRef<HTMLParagraphElement>(null);
  const websiteLinkRef = useRef<HTMLAnchorElement>(null);
  const similarPriceRefs = useRef<(HTMLDivElement | null)[]>([]);
  const similarPricePerSqmRefs = useRef<(HTMLParagraphElement | null)[]>([]);
  const footerLogoRef = useRef<HTMLDivElement>(null);
  const footerNameRef = useRef<HTMLSpanElement>(null);

  // Use the public API query
  const {
    data: property,
    isLoading,
    error
  } = usePublicSalesProperty(id!);

  // Get similar properties
  const {
    data: similarProperties = []
  } = usePublicSimilarSalesProperties(id!);

  // Get agency color
  const agencyColor = property?.agency?.primaryColor || '#f97316';
  ...
  const badgeGradient = generateBadgeGradient(agencyColor);

  // Update sidebar height based on content
  useEffect(() => {
  ...
  if (isLoading) {
    return <SalesListingSkeleton agencyColor={agencyColor} />;
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col items-center justify-center p-8">
        <div className="text-center max-w-md bg-white rounded-2xl p-8 shadow-xl border">
          <Home className="h-20 w-20 mx-auto mb-4" style={{ color: agencyColor }} />
          <h1 className="text-2xl font-bold mb-2 text-gray-800">Property Not Found</h1>
          <p className="text-gray-600 mb-6">
            The sales property you're looking for doesn't exist or is no longer available.
          </p>
          <Link href="/">
            <Button 
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `Check out this property for sale: ${property.title}`,
  ...
  const renderAmenities = () => {
    ...
    const commonAmenities = [
      'Parking', 'Garden', 'Pool', 'Air Conditioning', 'Heating',
      'Fireplace', 'Alarm', 'Elevator', 'Terrace', 'Balcony'
    ];

    return commonAmenities
      .filter(amenity => amenities.includes(amenity) || amenities.some(a => a.toLowerCase().includes(amenity.toLowerCase())))
      .slice(0, 12)
      .map((amenity, index) => (
        <div 
          key={index} 
            ...
            <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl overflow-hidden shadow-2xl group">
              {property.media.length > 0 ? (
                ...
                <div className="w-full h-[550px] flex items-center justify-center" style={{ background: gradientBg }}>
                  <Home className="h-32 w-32" style={{ color: agencyColor }} />
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <p className="text-lg font-medium text-gray-700">
                        <DollarSign className="inline h-4 w-4 mr-2" style={{ color: agencyColor }} />
                        €{property.pricePerSqm}/m²
                      </p>
                    </div>
                    <div className="text-right">
                      <Badge className="px-3 py-1.5 mb-2" style={{ background: `${agencyColor}20`, color: agencyColor }}>
                        <Target className="h-3 w-3 mr-1 inline" />
                        {property.propertyType}
                      </Badge>
                      <p className="text-sm text-gray-600">Excellent Value</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {[
                  ...
                  { icon: Car, label: 'Parking', value: property.amenities?.includes('Parking') ? 'Yes' : 'No', color: agencyColor },
                ].map((item, index) => (
                  <div 
                    key={index}
                ...
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500">Discover this beautiful property</p>
                    </div>
                  </div>
                  ...
                  <div className="p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500">Everything this property offers</p>
                      </div>
                    </div>
                    ...
                    {property.amenities.length > 12 && (
                      <div className="mt-6 pt-6 border-t" style={{ borderColor: `${agencyColor}20` }}>
                        <p className="text-sm font-medium" style={{ color: agencyColor }}>
                          +{property.amenities.length - 12} more features available
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="p-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {[
                      ...
                      { label: 'Status', value: property.status, badge: true },
                      { label: 'License', value: property.licenseNumber || 'Not specified' },
                      { label: 'Last Updated', value: new Date(property.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) },
                    ].map((item, index) => (
                      <div 
                        key={index}
                        className="p-4 rounded-xl transition-all duration-200 hover:shadow-sm"
                        style={{ backgroundColor: `${agencyColor}05` }}
                      >
                        <span className="text-sm font-medium text-gray-600 block mb-1">{item.label}</span>
                        {item.badge ? (
                          <Badge className="px-3 py-1" style={{ background: `${agencyColor}20`, color: agencyColor }}>
                            {item.value}
                          </Badge>
                        ) : (
                  ...
                  <div className="pt-8 border-t" style={{ borderColor: `${agencyColor}20` }}>
                    <h4 className="font-bold text-xl mb-6 flex items-center gap-2">
                      <Percent className="h-5 w-5" style={{ color: agencyColor }} />
                      Mortgage Information
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[
                        { 
                          ...
                          value: `€${property.mortgage.downPayment}`,
                          icon: DollarSign 
                        },
                        { 
                          label: 'Loan Amount', 
                          value: `€${property.mortgage.loanAmount}`,
                          icon: TrendingUp 
                        },
                        { 
                        ...
                        <div 
                          key={index}
                          className="text-center p-6 rounded-xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
                          style={{ 
                            backgroundColor: index % 2 === 0 ? `${agencyColor}05` : 'white',
                            borderColor: `${agencyColor}20`
                          }}
                    ...
                    <p className="text-xs text-gray-500 mt-4 text-center">
                      *Based on 30-year fixed mortgage at 3.5% interest rate. Your actual payment may vary.
                    </p>
                  </div>
                </div>
              </section>
                  ...
                  <div className="p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500">Explore all photos</p>
                      </div>
                          ...
                          <div className="relative">
                            {similar.thumbnail ? (
                              ...
                              <div className="w-full h-48 flex items-center justify-center"
                                style={{ background: gradientBg }}
                              >
                                <Home className="h-16 w-16" style={{ color: agencyColor }} />
                              </div>
                            )}
                          ...
                          <div className="p-6">
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-xs text-gray-500">€{similar.pricePerSqm}/m²</p>
                              </div>
                              <Button 
                                size="sm" 
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <Badge className="mt-1 px-2 py-0.5 text-xs" style={{ background: `${agencyColor}20`, color: agencyColor }}>
                          <Shield className="h-2 w-2 mr-1 inline" />
                          Trusted Partner
                        </Badge>
                      </div>
                    </div>
              ...
              >
                ...
                <CardContent className="p-6">
                  <h4 className="font-bold text-lg mb-6 flex items-center gap-2">
                    <Target className="h-5 w-5" style={{ color: agencyColor }} />
                    Quick Details
                  </h4>
...
function SalesListingSkeleton({ agencyColor = '#f97316' }: { agencyColor?: string }) {
