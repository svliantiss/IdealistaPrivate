...
export default function Dashboard() {
  ...
  const agentBookings = bookings.filter((b: any) => 
    b.ownerAgentId === CURRENT_AGENT_ID || b.bookingAgentId === CURRENT_AGENT_ID
  );
  
  // Calculate pending bookings that need action (pending status)
  const pendingBookings = agentBookings.filter((b: any) => b.status === 'pending').length;
  
  // Calculate sold houses
  ...
  const activeBookings = agentBookings.filter((b: any) => 
    b.status !== 'cancelled' && b.status !== 'archived'
  );
  
  // Filter out commissions for cancelled bookings
  const activeCommissions = commissions.filter((c: any) => {
    const booking = agentBookings.find((b: any) => b.id === c.bookingId);
    return booking && booking.status !== 'cancelled';
  });
  
  // Calculate total rental commission
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { 
              ...
              change: `${activeRentalListings} rental${activeRentalListings !== 1 ? 's' : ''}, ${activeSalesListings} sales`, 
              icon: Building, 
              color: "text-blue-500", 
            ...
            { 
              ...
              change: pendingBookings === 1 ? "1 needs action" : `${pendingBookings} need action`, 
              icon: Clock, 
            ...
            { 
              title: "Sold Houses", 
              value: soldHouses.toString(), 
              change: "Properties sold", 
              icon: Home, 
            ...
            { 
              ...
              link: "/rental-commissions",
              showChange: false
            },
            { 
              title: "Sales Commissions", 
              value: `€${totalSalesCommission.toFixed(0)}`, 
              change: "From sales", 
              icon: CreditCard, 
              ...
              <Card className="shadow-sm hover:shadow-md transition-shadow border-border/50 cursor-pointer">
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center mt-1">
                    {stat.showChange && stat.change.includes('+') && <ArrowUpRight className="h-3 w-3 mr-1 text-emerald-500" />}
                    {stat.change}
                  </p>
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
              {(() => {
                const recentBookings = agentBookings
                  .filter((b: any) => b.status !== 'cancelled' && b.status !== 'archived')
                  .slice(0, 4);
                
                if (recentBookings.length === 0) {
                  return <p className="text-sm text-muted-foreground text-center py-4">No recent bookings</p>;
                }
                
                return recentBookings.map((booking: any, i: number) => (
